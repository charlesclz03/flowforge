...
- **PROMPTS**: Session queues now avoid reusing the same rhyme family (for example `station` / `nation` / `creation`) until other available families have cycled.
- **PRACTICE**: Skill Check and Practice now support 2-bar word frequency alongside 4, 8, and 16 bars, with session-save/API paths accepting the value.
- **TTS**: Spoken prompts now fire immediately when the PLAYING prompt is displayed and cancel on pause/stop/word changes to prevent delayed or doubled speech.
...
- **AUDIO**: Live practice beat playback now uses the Web Audio gapless looper, preserving calibrated cue points at track end and continuing until the session stops or times out.
- **RECORDING**: Empty recording fallbacks now keep retryable save payloads, matching audio and stats-only completions when a transient save failure needs another attempt.
...
- **TEST**: Added targeted Vitest coverage for session queues, word-family rotation, 2-bar session saves, TTS timing, calibrated loop-start math, guarded profile-setup routing, username/profile-completion APIs, and iPad-class device detection.
...
- **FIX**: `attemptNavigation` now enforces "End Session?" only on `/practice` routes and auto-clears stale active flags outside practice.
- **FIX**: Bottom nav now no-ops when tapping the already active tab, preventing false guard prompts on pages like `/recordings`.
...
- **FIX**: Unified `/api/recordings` Pro-gating with signed-upload rules so SUPERADMIN users are no longer blocked by Free-tier storage checks (`403`) after successful signed uploads.
- **FIX**: Expanded `/api/upload/signed-url` missing-bucket detection to catch Supabase `404`/`related resource does not exist` responses, restoring bucket self-heal retries.
