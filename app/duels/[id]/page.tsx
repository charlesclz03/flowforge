...
export default async function DuelPage({ params }: DuelPageProps) {
  ...
  if (!session.parent) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen">
            ...
            <a href={`/practice?challengeId=${session.id}`} className="px-4 py-2 bg-accent-primary rounded-full text-black font-bold">
                Challenge this Beat!
            </a>
        </div>
    )
  ...
  const parentVoteCount = await prisma.duelVote.count({
    where: {
      duelId: session.parentId!, // The context
      votedForId: session.parentId! // The winner
    }
  })
