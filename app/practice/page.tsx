...
export default function PracticePage() {
    ...
    async (blob: Blob, recordedDuration: number) => {
      ...
      if (isInfiniteMode) {
        toast('Session Completed (Practice Mode)', {
          icon: '♾️',
          style: { background: 'linear-gradient(to right, #6b21a8, #c026d3)', color: '#fff' },
        })
        return
      }
  ...
  const paramsRef = useRef({ frequency, wordList, selectedBeat, sessionDuration, isTTSEnabled })

  useEffect(() => {
    paramsRef.current = { frequency, wordList, selectedBeat, sessionDuration, isTTSEnabled }
